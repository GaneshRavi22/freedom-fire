...
Two-step process: keyword pass first (fast, always runs), then a Claude semantic pass that
corrects and enriches the result. Claude also generates the spending insights.

#### Step 1 — Keyword pass (initial + fallback)

Case-insensitive substring match. First matching category wins.
Categories checked in order: food, transport, shopping, health, entertainment, utilities,
emi_loans, investments, other.
...
emi_loans:     emi, loan, repayment, installment, instalment, hdfc loan, icici loan,
               sbi loan, axis loan, mortgage
investments:   sip, mutual fund, mf, nps, ppf, elss, groww, zerodha, kite, upstox,
               smallcase, coin, scripbox, paytm money, stock, demat
other:         (catch-all — no keywords)
```

#### Step 2 — Claude semantic pass (replaces keyword result; falls back silently on error)

After the keyword pass, all unique merchant descriptions are sent to Claude Haiku via forced
tool use (`analyze_transactions`). Claude returns:

1. A `{description → category}` map covering the same 9 categories above. Each returned
   category is validated against the allowed set; unknown values are dropped.
2. An `insights` array of 3–5 strings grounded in actual ₹ amounts from the breakdown.

Claude improves on keyword matching for:
- Merchants that fall into `emi_loans` or `investments` but aren't matched by keywords
- Ambiguous descriptions (e.g. "HDFC Bank" could be a bill payment or EMI)
- "other" transactions where the merchant name implies a known category

**Fallback:** if the Claude call fails (any error), keyword categories and the hardcoded
`generateInsights()` benchmarks are used unchanged. The response format is identical in
both cases — the caller cannot distinguish which path ran.
...
**Primary path (Claude):** 3–5 insights generated by Claude as part of the `analyze_transactions`
tool call. Insights reference actual ₹ amounts from the user's breakdown, are actionable for
FIRE planning, and use Indian financial context (SIP, EMI, corpus, freedom days).

**Fallback path (hardcoded benchmarks):** used only when Claude is unavailable.
...
```typescript
{
  ...
  insights: string[],   // Claude-generated when available; hardcoded fallback otherwise
...
- `ANTHROPIC_API_KEY` — required for Claude categorisation + insights; function still works
  without it (falls back to keyword matching and hardcoded insights)
...
- [ ] categoryBreakdown may contain emi_loans and investments keys (not just the original 6)
- [ ] If Claude call fails, response is identical to keyword-only path (no error exposed to caller)
- [ ] Claude-returned category values outside the allowed 9 are silently discarded (fallback to keyword)
